...
public class TelevisorMetodos {
   ...
   public static int precioVenta=0;
    ...
    public static void venderTele(){
            int buscar=0;
            System.out.println("ingrese referencia a vender.");
            int buscarReferencia= input.nextInt();
            for (TelevisorCons televisor:televisoresRegistrados) {
                if (televisor.getReferencia()==buscarReferencia){
                    System.out.println("ingrese cuantos televisores va a vender");
                    int vender= input.nextInt();
                    buscar=1;
                    if (vender==1){
                        double totalV=precioVenta*0.10;
                        System.out.println("el valor del televisor tiene un costo de "+ (precioVenta+totalV));

                        System.out.println("televisor vendido exitosamenete.");

                    }else {
                        System.out.println("el valor de los televisores es de "+ (precioVenta*vender));
